...
// //   writeToDAC(LEDBus, DAC_ADDRESS, DAC_DATA_REGISTER, DAC_DATA_ON_MSB, DAC_DATA_ON_LSB);

//   writeToDAC(LEDBus, DAC_ADDRESS, GEN_CONFIG_REGISTER, GEN_CONFIG_MSB, GEN_CONFIG_LSB);

//   writeToDAC(LEDBus, DAC_ADDRESS, DAC_DATA_REGISTER, DAC_DATA_ON_MSB, DAC_DATA_ON_LSB);

// //   LEDBus->write(DAC_DATA_REGISTER);
// //   Serial.printf("Transmission was: %d\n", LEDBus->endTransmission());
// //   LEDBus->write(DAC_DATA_ON >> 2);
// //   LEDBus->write( (DAC_DATA_ON & 0x03) << 6 );

// //   Serial.printf("Transmission was: %d\n", LEDBus->endTransmission());

// //   writeToDAC(LEDBus, DAC_ADDRESS, TRIGGER_REGISTER, TRIGGER_MSB, TRIGGER_LSB);


//   Serial.println("Reading DAC_Data Register");
//   readRegisters(LEDBus, DAC_DATA_REGISTER);
//   Serial.println("Reading config Register");
//   readRegisters(LEDBus, GEN_CONFIG_REGISTER);
//   Serial.println("Reading trigger Register");
//   readRegisters(LEDBus, TRIGGER_REGISTER);

//   // LEDBus->beginTransmission(DAC_ADDRESS);
//   // LEDBus->write(DAC_DATA_REGISTER);
//   // LEDBus->endTransmission();
//   // LEDBus->requestFrom(DAC_ADDRESS, 2); // We want one byte
//   // uint16_t val = LEDBus->read();
//   // Serial.printf("DAC_DATA REGISTER STATE: %d\n", val);
...
//   writeToDAC(LEDBus, DAC_ADDRESS, DAC_DATA_REGISTER, DAC_DATA_OFF, DAC_DATA_OFF);

// //     LEDBus->beginTransmission(DAC_ADDRESS); 

// //   LEDBus->write(DAC_DATA_REGISTER);
// //   LEDBus->write(DAC_DATA_OFF >> 2);
// //   LEDBus->write( (DAC_DATA_OFF & 0x03) << 6 );
