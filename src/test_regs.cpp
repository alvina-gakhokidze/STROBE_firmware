...
//    //writeToDAC(&blueLEDBus, DAC_ADDRESS, TRIGGER_REGISTER, SAVE_TO_NVM_MSB, SAVE_TO_NVM_LSB );
    ...
    registerTalk::writeToDAC(&blueLEDBus, DAC_ADDRESS, GEN_CONFIG_REGISTER, GEN_CONFIG_ON_MSB, GEN_CONFIG_ON_LSB );
    ...
    registerTalk::readRegisters(&blueLEDBus, DAC_ADDRESS, GEN_CONFIG_REGISTER);
    Serial.println("reading dac_data register");
    registerTalk::readRegisters(&blueLEDBus, DAC_ADDRESS, DAC_DATA_REGISTER);
    Serial.println("Reading trigger register");
    registerTalk::readRegisters(&blueLEDBus, DAC_ADDRESS, TRIGGER_REGISTER);
...
void loop() {
    ...
    timestamp = millis();
    //while(millis()-timestamp < 1);
    registerTalk::writeToDAC(&blueLEDBus, DAC_ADDRESS, DAC_DATA_REGISTER, DAC_DATA_OFF, DAC_DATA_OFF);
    timestamp = millis();
    //while(millis()-timestamp < 10);
    registerTalk::writeToDAC(&blueLEDBus, DAC_ADDRESS, DAC_DATA_REGISTER, DAC_DATA_HALF_POWER_MSB, DAC_DATA_HALF_POWER_LSB);
